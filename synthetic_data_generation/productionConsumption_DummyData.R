...
data.month <- c(300, 300, 300, 300, 300, 300, 300, 300, 300, 300, 300, 300)
...
data.hour.total = (data.hour1 + data.hour2)
...
# Plotting  
par(mfrow = c(1,1))
plot(data.hour.total[1:240], type = "l", main = "January") * 0.5
...
productionConsumption = round(data.hour.total.rand, digits = 4)
...
output["productionConsumption (kWh)"] <- as.data.frame(productionConsumption)

# Write Output to CSV
output
write.csv(output,"productionConsumption_dummyData.csv", row.names = FALSE)